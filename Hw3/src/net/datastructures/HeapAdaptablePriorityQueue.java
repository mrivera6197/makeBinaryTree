...
public class HeapAdaptablePriorityQueue<K,V> extends HeapPriorityQueue<K,V>
  ...
  //The ArrListIterator class is defined below
  ...
  private class PQIterator implements Iterator<Entry<K,V>>
  {

    //Current pointer in the list
    private int curr;

    //Public facing constructor - starts at beginning of list
    public PQIterator()
    {
      this.curr=0;
    }

    //Is there a next element after curr?
    public boolean hasNext()
    {
      return this.curr < heap.size();
    }

    //Get next element after curr and update curr
    public Entry<K,V> next()
    {
      if(curr==heap.size())
        throw new NoSuchElementException("No next element");
      Entry<K,V> temp = heap.get(curr);
      curr++;
      return temp;
    }
